#ifndef ____INDEXSTRUCTURES_H_INCLUDED____
#define ____INDEXSTRUCTURES_H_INCLUDED____
...
// - AllocNode/FreeNode 
// - CheckInNode(nodeId): returns a pointer to a TNode instance containing the contents of the given node.
...
// book-keeping information, but relies on two helped classes (TInternalStore,
...
template <
	typename TKey_, typename TDat_, typename TComp_, 
	typename TNodeId_, typename TInternalStore_, typename TLeafStore_>
class TBtreeOps
{
private:
...
public:
	...
	TBtreeOps(
		...
		internalStore(InternalStore), leafStore(LeafStore), internalCapacity(InternalCapacity), leafCapacity(LeafCapacity),
...
//----------------------------------------------------------------------------
	...
	// split or merged).
	TKeyLocation Add(const TKey& key, const TDat& dat)
	{
		TVec<TNodeId> branch; 
		Assert(nLevels >= 1);
		...
		if (pRoot->v.Empty())
		{
			...
			pLeaf->Clr(); int idx = pLeaf->v.Add(TLeafNode::TKd(key, dat));
			nLevels++; first.Add(leaf); last.Add(leaf);
			// Add the leaf to the root.
			pRoot->v.Add(TInternalNode::TKd(key, leaf));
			pRoot.SetDirty();
		...
		PLeafNode pLeaf2(leafStore, leaf2, true);
		{
			...
			{
				TLeafNode::TKd &kdDest = (dest >= leafCapacity) ? pLeaf2->v[dest - leafCapacity] : pLeaf->v[dest];
		...
		for (int level = nLevels - 2; level >= 0; level--)
		{
			...
			{
				TInternalNode::TKd &kdDest = (dest >= internalCapacity) ? pCur2->v[dest - internalCapacity] : pCur->v[dest];
...
protected:
	...
	void DelHelper(PNode &pLeft, PNode &pRight, int capacity, bool &deleted)
	{
		...
		int nLeft2 = pLeft->v.Len(), nRight2= pRight->v.Len();
		Assert(nLeft2 >= capacity); Assert(nRight2 >= capacity); Assert(nLeft2 + nRight2 == nLeft + nRight);
	}
...
protected:
	...
	int RangeQuery_Recursion(TRangeQueryData<TSink> &Q, TNodeId node, int level) const
	{
		...
		if (level < nLevels - 1)
		{
			...
			const TInternalNode::TKdV &v = pNode->v; int n = v.Len();
			for (int i = 0; i < n; i++)
			{
		...
		else
		{
			...
			const TLeafNode::TKdV &v = pNode->v; int n = v.Len();
			for (int i = 0; i < n; i++)
			{
				int c = cmp(v[i].Key, Q.minKey);
				if (c < 0 || (c == 0 && ! Q.includeMin))
...
protected:
	...
	{
		static TPrinter<PNode::TNode::TKey> keyPrinter;
		static TPrinter<PNode::TNode::TDat> datPrinter;
...
public:
	...
	// Dummy method for compatibility with TParanoidBtreeNodeStore.
...
public:
	...
	void CheckInNode(const TNodeId& nodeId, TNode *node, bool dirty)
	{
		...
		if (! dirty) 
			...
			// Store the modifier node from w.checkedOutNode back into w.node.  We use the copy constructor
...
#endif // ____INDEXSTRUCTURES_H_INCLUDED____